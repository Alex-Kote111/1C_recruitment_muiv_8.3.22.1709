...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ДанныеФотографии.Выбран() Тогда
		Объект.ФотоКандидата = ПоместитьВоВременноеХранилище(ДанныеФотографии.Фотография.Получить(), УникальныйИдентификатор);
	КонецЕсли; 
...
Процедура ЗапланироватьМероприятие(Команда)
	...
	ОткрытьФорму("Обработка.ПланированиеТелефонныхИнтервьюИСобеседований.Форма.Форма",, ЭтотОбъект);
