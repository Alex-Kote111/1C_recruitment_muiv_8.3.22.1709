                              
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	
	Если ДанныеЗаполнения <> Неопределено И ДанныеЗаполнения.ИсточникЗаполнения = "ПоискИПросмотрРезюмеХХ" Тогда 			
		ЗаполнениеМестаПроживания(ДанныеЗаполнения);				
		ЗаполнениеУровняОбразования(ДанныеЗаполнения);		
		ЗаполнениеТЧОпытРаботы(ДанныеЗаполнения);		
		ЗаполнениеТЧЗнанияЯзыков(ДанныеЗаполнения);
		ЗаполнениеТЧПодходящиеГрафикиРаботы(ДанныеЗаполнения);
		ЗаполнениеТЧПодходящиеТипыЗанятости(ДанныеЗаполнения);
		ЗаполнениеТЧКлючевыеНавыки(ДанныеЗаполнения);
		ЗаполнениеДанныхОбОбразовании(ДанныеЗаполнения);
	ИначеЕсли ДанныеЗаполнения <> Неопределено И ДанныеЗаполнения.ИсточникЗаполнения = "ЗагрузкаОткликовССайтаУниверситета" Тогда
		ЗаполнениеДанныхКандидатаССайта(ДанныеЗаполнения);	
	КонецЕсли;
	         
КонецПроцедуры  

Процедура ЗаполнениеМестаПроживания(ДанныеЗаполнения)
	
	Если ДанныеЗаполнения.ГородПроживания <> Неопределено Тогда
		СсылкаНаГород = Справочники.Города.НайтиПоРеквизиту("ИдентификаторГородаХХ", ДанныеЗаполнения.ГородПроживания["id"]);
		Если НЕ СсылкаНаГород.Пустая() Тогда
			Город = СсылкаНаГород;
			Регион = Город.Родитель.Наименование;
		Иначе
			Город = ДанныеЗаполнения.ГородПроживания["name"]; 	
		КонецЕсли;
	КонецЕсли;
			
КонецПроцедуры

Процедура ЗаполнениеУровняОбразования(ДанныеЗаполнения)	
	
	Образование = Перечисления.УровниОбразования[СтрЗаменить(ТРег(ДанныеЗаполнения.УровеньОбразованияКандидата["name"])," ","")]
	
КонецПроцедуры

Процедура ЗаполнениеТЧОпытРаботы(ДанныеЗаполнения)

	ОпытРаботыВОрганизациях = ДанныеЗаполнения.ОпытРаботы; 
	
	Для Каждого Опыт Из ОпытРаботыВОрганизациях Цикл
		СтрокаТЧ = ОпытРаботы.Добавить();
		
		Если Опыт["area"] <> Неопределено Тогда 
			СтрокаТЧ.РасположениеОрганизации = Опыт["area"]["name"];
		КонецЕсли; 
		
		СтрокаТЧ.НазваниеОрганизации = Опыт["company"];
		СтрокаТЧ.СайтКомпании = Опыт["company_url"];
		
		Если Опыт["industry"] <> Неопределено Тогда
			СтрокаТЧ.ОтрасльКомпании = Опыт["industry"]["name"];
		КонецЕсли;
                                  
		СтрокаТЧ.НачалоРаботы = Дата(СтрЗаменить(Сред(Опыт["start"], 0, 10), "-", ""));
		
		Если Опыт["end"] <> Неопределено Тогда
			СтрокаТЧ.ОкончаниеРаботы = Дата(СтрЗаменить(Сред(Опыт["end"], 0, 10), "-", ""));
		КонецЕсли;
		
		СтрокаТЧ.Должность = Опыт["position"];		
	КонецЦикла;

КонецПроцедуры

Процедура ЗаполнениеТЧЗнанияЯзыков(ДанныеЗаполнения)
	
	СписокЯзыков = ДанныеЗаполнения.ЗнанияЯзыков;	
	Для Каждого Язык Из СписокЯзыков Цикл		
		СсылкаНаЯзык = Справочники.Языки.НайтиПоНаименованию(Язык["name"]);	
		СтрокаТЧ = ЗнанияЯзыков.Добавить(); 
		Если НЕ СсылкаНаЯзык.Пустая() Тогда
			СтрокаТЧ.Язык = СсылкаНаЯзык;
		Иначе
			СтрокаТЧ.Язык = Язык["name"];
		КонецЕсли;
				
		УровеньВладения = Язык["level"]["name"];
		Для каждого ТекЭлемент Из Метаданные.Перечисления["УровниЗнанийЯзыков"].ЗначенияПеречисления  Цикл
			Если ТекЭлемент.Синоним = УровеньВладения Тогда
				СтрокаТЧ.УровеньВладения = Перечисления["УровниЗнанийЯзыков"][ТекЭлемент.Имя];
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнениеТЧПодходящиеГрафикиРаботы(ДанныеЗаполнения)

	СписокПодходящихГрафиков = ДанныеЗаполнения.ПодходящиеГрафикиРаботы;
	
	Для Каждого График Из СписокПодходящихГрафиков Цикл
		СтрокаТЧ = ПодходящиеГрафикиРаботы.Добавить();
		ПодходящийГрафикРаботы = График["name"];
		Для каждого ТекЭлемент Из Метаданные.Перечисления["ГрафикиРаботы"].ЗначенияПеречисления  Цикл
			Если ТекЭлемент.Синоним = ПодходящийГрафикРаботы Тогда
				СтрокаТЧ.ГрафикРаботы = Перечисления["ГрафикиРаботы"][ТекЭлемент.Имя];
				Прервать;
			КонецЕсли;
		КонецЦикла;			
	КонецЦикла;

КонецПроцедуры

Процедура ЗаполнениеТЧПодходящиеТипыЗанятости(ДанныеЗаполнения)

	СписокПодходящихТиповЗанятости = ДанныеЗаполнения.ПодходящиеТипыЗанятости;
	
	Для Каждого ТипЗанятости Из СписокПодходящихТиповЗанятости Цикл
		СтрокаТЧ = ПодходящиеТипыЗанятости.Добавить();
		ПодходящийТипЗанятости = ТипЗанятости["name"];
		Для каждого ТекЭлемент Из Метаданные.Перечисления["ТипыЗанятости"].ЗначенияПеречисления  Цикл
			Если ТекЭлемент.Синоним = ПодходящийТипЗанятости Тогда
				СтрокаТЧ.ТипЗанятости = Перечисления["ТипыЗанятости"][ТекЭлемент.Имя];
				Прервать;
			КонецЕсли;
		КонецЦикла;			
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнениеТЧКлючевыеНавыки(ДанныеЗаполнения)
	
	СписокНавыков = ДанныеЗаполнения.КлючевыеНавыки;	
	Для Каждого Навык Из СписокНавыков Цикл		
		СтрокаТЧ = КлючевыеНавыки.Добавить();
		СтрокаТЧ.Навык = Навык; 		
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнениеДанныхКандидатаССайта(ДанныеЗаполнения)
		
	СсылкаНаГород = Справочники.Города.НайтиПоНаименованию(ДанныеЗаполнения.Город);
	Если НЕ СсылкаНаГород.Пустая() Тогда
		Регион = СсылкаНаГород.Родитель;
		Город = СсылкаНаГород;
	КонецЕсли; 
	
	СсылкаНаМетро = Справочники.СтанцииМетро.НайтиПоНаименованию(ДанныеЗаполнения.СтанцияМетро); 
	Если НЕ СсылкаНаМетро.Пустая() Тогда
		СтанцияМетро = СсылкаНаМетро;
	КонецЕсли;
	Образование = Перечисления.УровниОбразования[СтрЗаменить(ТРег(ДанныеЗаполнения.Образование)," ","")];
	НаименованиеВакансии = ДанныеЗаполнения.СсылкаНаВакансию.НаименованиеВакансии;
	Если ДанныеЗаполнения.Свойство("ДвоичныеДанныеФайла") Тогда
		ФотоКандидата = ПоместитьВоВременноеХранилище(ДанныеЗаполнения.ДвоичныеДанныеФайла);
	КонецЕсли;

КонецПроцедуры

Процедура ЗаполнениеДанныхОбОбразовании(ДанныеЗаполнения)
	
	СписокКурсов = ДанныеЗаполнения.ОбразованиеСоискателя["additional"];
	СписокТестов = ДанныеЗаполнения.ОбразованиеСоискателя["attestation"];
	СписокСреднегоОбразования = ДанныеЗаполнения.ОбразованиеСоискателя["elementary"];
	СписокВысшегоОбразования = ДанныеЗаполнения.ОбразованиеСоискателя["primary"];
		
	Для Каждого Курс Из СписокКурсов Цикл
		НоваяСтрока = КурсыПовышенияКвалификации.Добавить();
		НоваяСтрока.НазваниеКурсаТеста = Курс["name"];
		НоваяСтрока.Организация = Курс["organization"];
		НоваяСтрока.СпециальностьСпециализация = Курс["result"];
		НоваяСтрока.ГодОкончания = Курс["year"];
	КонецЦикла;
	
	Для Каждого Тест Из СписокТестов Цикл
		НоваяСтрока = ПройденныеТестыИлиЭкзамены.Добавить();
		НоваяСтрока.НазваниеКурсаТеста = Тест["name"];
		НоваяСтрока.Организация = Тест["organization"];
		НоваяСтрока.СпециальностьСпециализация = Тест["result"];
		НоваяСтрока.ГодОкончания = Тест["year"];
	КонецЦикла;
	
	Для Каждого Элемент Из СписокСреднегоОбразования Цикл
		НоваяСтрока = СреднееОбразование.Добавить();
		НоваяСтрока.НазваниеУчебногоЗаведения = Элемент["name"];
		НоваяСтрока.ГодОкончания = Элемент["year"];
	КонецЦикла;
	
	Для Каждого Элемент Из СписокВысшегоОбразования Цикл
		НоваяСтрока = ОбразованиеВышеСреднего.Добавить();		
		Для каждого ТекЭлемент Из Метаданные.Перечисления["УровниОбразования"].ЗначенияПеречисления Цикл
			Если ТекЭлемент.Синоним = Элемент["education_level"]["name"] Тогда
				НоваяСтрока.УровеньОбразования = Перечисления["УровниОбразования"][ТекЭлемент.Имя];
				Прервать;
			КонецЕсли;
		КонецЦикла;		
		НоваяСтрока.НазваниеУчебногоЗаведения = Элемент["name"];
		НоваяСтрока.Факультет = Элемент["organization"];
		НоваяСтрока.СпециальностьСпециализация = Элемент["result"];
		НоваяСтрока.АкронимУчебногоЗаведения = Элемент["university_acronym"];
		НоваяСтрока.ГодОкончания = Элемент["year"];
	КонецЦикла;	

КонецПроцедуры

