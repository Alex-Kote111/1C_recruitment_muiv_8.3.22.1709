...
Процедура ЗагрузитьКандитовВТЗ(ОткликиКандидатов) 
	...
	Для Каждого Кандидат Из ОткликиКандидатов Цикл
		...
		СсылкаНаКандидата = Справочники.Кандидаты.НайтиПоРеквизиту("ИдентификаторНаСайтеУниверситета", Кандидат.id);
		
		Если СкрытьДобавленныхКандидатов Тогда	
			Если НЕ СсылкаНаКандидата.Пустая() Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		НоваяСтрока.УжеДобавлен = ?(СсылкаНаКандидата.Пустая(), Ложь, Истина);
	...
	ВсегоОткликов = ОткликиКандидатов.Количество();
