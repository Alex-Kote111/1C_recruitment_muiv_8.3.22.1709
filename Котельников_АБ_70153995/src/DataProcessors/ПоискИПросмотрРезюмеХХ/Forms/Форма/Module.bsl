...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтотОбъект.ВладелецФормы <> Неопределено Тогда
		ЗаполнитьПараметрыПоискаПоВакансии(ЭтотОбъект.ВладелецФормы.Объект);	
	КонецЕсли;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ИзменитьЗаголовокГруппыНаСервере(СписокГородов, Элементы.ГруппаПодборГорода, "Город", "Место проживания", ИспользоватьМестоПроживания);
	ИзменитьЗаголовокГруппыНаСервере(СписокСтатусовПоискаРаботы, Элементы.ГруппаСтатусПоискаРаботы, "Статус", "Статус поиска работу", ИспользоватьСтатусПоискаРаботы);
	ИзменитьЗаголовокГруппыНаСервере(СписокГрафиковРаботы, Элементы.ГруппаПодборГрафикаРаботы, "ГрафикРаботы", "График работы", ИспользоватьГрафикРаботы);
	ИзменитьЗаголовокГруппыНаСервере(СписокТиповЗанятости, Элементы.ГруппаПодборТипаЗанятости, "ТипЗанятости", "Тип занятости", ИспользоватьТипЗанятости);
	ИзменитьЗаголовокГруппыНаСервере(СписокВариантовОпытаРаботы, Элементы.ГруппаПодборОпытаРаботы, "ОпытРаботы", "Опыт работы", ИспользоватьОпытРаботы);
	ИзменитьЗаголовокГруппыНаСервере(СписокВариантовДополнительногоФильтра, Элементы.ГруппаДополнительныеФильтрыПодбор, "Фильтр", "Дополнительный фильтр", ИспользоватьДопФильтр);
	ИзменитьЗаголовокГруппыНаСервере(СписокСтанцийМетро, Элементы.ГруппаПодборСтанцииМетро, "Станция", "Метро", ИспользоватьСтанциюМетро);
	ИзменитьЗаголовокГруппыНаСервере(СписокУровнейОбразования, Элементы.ГруппаПодборОбразования, "Образование", "Образование", ИспользоватьОбразование);
	ИзменитьЗаголовокГруппыНаСервере(СписокЯзыков, Элементы.ГруппаПодборЯзыков, "Язык", "Знание языков", ИспользоватьЯзык);
	ИзменитьЗаголовокГруппыНаСервере(СписокКлючевыхНавыков, Элементы.ГруппаПодборКлючевыхНавыков, "Навык", "Ключевые навыки", ИспользоватьКлючевыеНавыки);
...
&НаКлиенте
Асинх Процедура ВакансияПриИзменении(Элемент)
	
	ОтветПользователя = Ждать ВопросАсинх("Параметры поиска будут очищены. Хотите продолжить?", РежимДиалогаВопрос.ДаНет,,, "Параметры будут очищены");
	...
	ДанныеВакансии = ПолучитьДанныеВакансии(Вакансия);
	ЗаполнитьПараметрыПоискаПоВакансии(ДанныеВакансии);
...
Процедура СозданиеЭлементаСправочникаКандидата(ОтветJSON)
	...
	ЗначенияРеквизитов.Вставить("ИдентификаторРезюмеХХ", ОтветJSON["id"]);
	...
	МассивКлючевыхНавыков = ОтветJSON["skill_set"];
	ЗначенияРеквизитов.Вставить("КлючевыеНавыки", МассивКлючевыхНавыков);
...
Функция ПолучитьДанныеВакансии(Вакансия)
	...
	Запрос.Текст = 
		...
		|	Вакансия.Образование КАК Образование,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВакансияКлючевыеНавыки.Навык КАК Навык
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВакансияЗнанияЯзыков.Язык КАК Язык
		|ИЗ
	...
	ДанныеНавыков = РезультатЗапроса[1].Выбрать();
	ДанныеЯзыков = РезультатЗапроса[2].Выбрать();
	
	ДанныеВакансии = Новый Структура;
	
	ДанныеШапки.Следующий();
	ДанныеВакансии.Вставить("НаименованиеВакансии", ДанныеШапки.НаименованиеВакансии);
	ДанныеВакансии.Вставить("Образование", ДанныеШапки.Образование);
	ДанныеВакансии.Вставить("ОпытРаботы", ДанныеШапки.ОпытРаботы);
	ДанныеВакансии.Вставить("ТипЗанятости", ДанныеШапки.ТипЗанятости);
	ДанныеВакансии.Вставить("ГрафикРаботы", ДанныеШапки.ГрафикРаботы);
	ДанныеВакансии.Вставить("ОплатаТрудаОт", ДанныеШапки.ОплатаТрудаОт);
	ДанныеВакансии.Вставить("ОплатаТрудаДо", ДанныеШапки.ОплатаТрудаДо);
	
	МассивНавыков = Новый Массив;
	Пока ДанныеНавыков.Следующий() Цикл
		МассивНавыков.Добавить(Новый Структура("Навык", ДанныеНавыков.Навык));	
	КонецЦикла;
	
	МассивЯзыков = Новый Массив;
	Пока ДанныеЯзыков.Следующий() Цикл
		МассивЯзыков.Добавить(Новый Структура ("Язык", ДанныеЯзыков.Язык));	
	КонецЦикла;	

	ДанныеВакансии.Вставить("КлючевыеНавыки", МассивНавыков);
	ДанныеВакансии.Вставить("ЗнанияЯзыков", МассивЯзыков);
...
&НаСервере
Процедура ИзменитьЗаголовокГруппыНаСервере(Список, ЭлементГруппа, НазваниеКолонки, НаименованиеЗаголовка, ИспользоватьПараметр)
...
&НаКлиенте
Процедура ЗаполнитьПараметрыПоискаПоВакансии(ДанныеВакансии)

	Если ДанныеВакансии.Свойство("Ссылка") Тогда
		Вакансия = ДанныеВакансии.Ссылка;
	КонецЕсли;
	
	НаименованиеРезюме = ДанныеВакансии.НаименованиеВакансии;
	
	СтрокаТЗ = СписокУровнейОбразования.Добавить();
	СтрокаТЗ.Образование = ДанныеВакансии.Образование;
		
	МассивНавыков = Новый Массив;	
	Для Каждого Навык Из ДанныеВакансии.КлючевыеНавыки Цикл
		МассивНавыков.Добавить(Навык.Навык);		
	КонецЦикла;
	
	МассивЯзыков = Новый Массив;	
	Для Каждого Язык Из ДанныеВакансии.ЗнанияЯзыков Цикл
		МассивЯзыков.Добавить(Язык.Язык);	
	КонецЦикла;
	
	ПодходящиеНавыкиИЯзыки = ПолучитьПодходящиеНавыкиИЯзыки(МассивНавыков, МассивЯзыков, Вакансия); // отбираем навыки и языки у которых заполнен идентификатор
	
	Для Каждого Навык Из ПодходящиеНавыкиИЯзыки[0] Цикл
		СтрокаТЗ = СписокКлючевыхНавыков.Добавить();
		СтрокаТЗ.Навык = Навык.Наименование;
		СтрокаТЗ.ИдентификаторНавыка = Навык.Идентификатор;
	КонецЦикла;
	
	Для Каждого Язык Из ПодходящиеНавыкиИЯзыки[1] Цикл
		СтрокаТЗ = СписокЯзыков.Добавить();
		СтрокаТЗ.Язык = Язык.Наименование;
		СтрокаТЗ.ИдентификаторЯзыка = Язык.Идентификатор;
		СтрокаТЗ.УровеньЗнания = Язык.УровеньВладения;
	КонецЦикла;
		
	СтрокаТЗ = СписокТиповЗанятости.Добавить();
	СтрокаТЗ.ТипЗанятости = ДанныеВакансии.ТипЗанятости;
	
	СтрокаТЗ = СписокГрафиковРаботы.Добавить();
	СтрокаТЗ.ГрафикРаботы = ДанныеВакансии.ГрафикРаботы;
	
	СтрокаТЗ = СписокВариантовОпытаРаботы.Добавить();
	СтрокаТЗ.ОпытРаботы = ДанныеВакансии.ОпытРаботы;
	
	ЖелаемаяЗарплатаОт = ДанныеВакансии.ОплатаТрудаОт;
	ЖелаемаяЗарплатаДо = ДанныеВакансии.ОплатаТрудаДо;
...
&НаСервереБезКонтекста
Функция ПолучитьПодходящиеНавыкиИЯзыки(МассивНавыков, МассивЯзыков, Вакансия)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	КлючевыеНавыки.Ссылка В(&МассивНавыков)
		|	И КлючевыеНавыки.ИдентификаторНавыкаХХ <> """"
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	Документ.Вакансия.ЗнанияЯзыков КАК ВакансияЗнанияЯзыков
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Языки КАК Языки
		|		ПО ВакансияЗнанияЯзыков.Язык = Языки.Ссылка
		|ГДЕ
		|	Языки.Ссылка В(&МассивЯзыков)
		|	И Языки.ИдентификаторЯзыкаХХ <> """"
		|	И ВакансияЗнанияЯзыков.Ссылка = &Вакансия";
	
	Запрос.УстановитьПараметр("МассивНавыков", МассивНавыков);
	Запрос.УстановитьПараметр("МассивЯзыков", МассивЯзыков);
	Запрос.УстановитьПараметр("Вакансия", Вакансия);
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	НайденныеНавыки = РезультатЗапроса[0];
	НайденныеЯзыки = РезультатЗапроса[1];
	
	СписокНавыков = Новый Массив;	
	Выборка = НайденныеНавыки.Выбрать();
	Пока Выборка.Следующий() Цикл
		СписокНавыков.Добавить(Новый Структура("Наименование, Идентификатор", Выборка.Наименование, Выборка.ИдентификаторНавыкаХХ));	
	КонецЦикла;
	
	СписокЯзыков = Новый Массив;	
	Выборка = НайденныеЯзыки.Выбрать();
	Пока Выборка.Следующий() Цикл
		СписокЯзыков.Добавить(Новый Структура("Наименование, Идентификатор, УровеньВладения", Выборка.Наименование, Выборка.ИдентификаторЯзыкаХХ, Выборка.УровеньВладения));	
	КонецЦикла;
	
	ПодходящиеНавыкиИЯзыки = Новый Массив;
	ПодходящиеНавыкиИЯзыки.Добавить(СписокНавыков);	
	ПодходящиеНавыкиИЯзыки.Добавить(СписокЯзыков);	
	
	Возврат ПодходящиеНавыкиИЯзыки;
...
Процедура ОчиститьРеквизитыФормы()
	...
	НаименованиеРезюме = "";
	СписокМестВстречиСлов.Очистить();
	ЗаполнитьЗначенияПоУмолчанию();
	СписокСтатусовПоискаРаботы.Очистить();
	ПериодПоискаРезюме = '00010101';
	ВозрастНижняяГраница = 0;
	ВозрастВерхняяГраница = 0;
	ВсеРезюмеСУказаннымВозрастом = Ложь;
	ПолСоискателя = ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.ПустаяСсылка");
	СписокУровнейОбразования.Очистить();
	СписокКлючевыхНавыков.Очистить();
	СписокЯзыков.Очистить();
	СписокТиповЗанятости.Очистить();
	СписокГрафиковРаботы.Очистить();
	СписокВариантовОпытаРаботы.Очистить();
	СписокГородов.Очистить();
	СписокСтанцийМетро.Очистить();
	СписокВариантовДополнительногоФильтра.Очистить();
	ЖелаемаяЗарплатаОт = 0;
	ЖелаемаяЗарплатаДо = 0;
	ЖелаемаяЗарплатаУказана = Ложь;
	НомерСтраницы = 0;
