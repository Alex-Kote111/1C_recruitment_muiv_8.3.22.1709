...
Асинх Процедура ОтправитьУведомлениеКандидату(Команда)
	...
	Если ДанныеДляОтправкиПисьма.ШаблонПисьма.Пустая() Тогда
		ОбщегоНазначенияКлиент.ВывестиСообщение("Не найден шаблон письма об отказе. Проверьте свои настройки пользователя",, Объект.АвторДокумента);
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		ОбщегоНазначенияКлиент.ВывестиСообщение("Кандидату """ + ДанныеДляОтправкиПисьма.ФИОКандидата + """ успешно отправлено уведомление об отказе");
		Объект.КандидатУведомлен = Истина;
...
Функция ПолучитьПоследнееУведомлениеКандидата(Вакансия, Кандидат)
	...
	ОтборЗаписи.Вставить("ЭтапУведомления", Перечисления.ЭтапыУведомлений.Отказ);
...
Функция ПолучитьДанныеДляОтправкиПисьма(Кандидат, Вакансия)
	...
	Для Каждого Настройка Из ТекПользователь.НастройкиУведомлений Цикл
		Если Настройка.ЭтапУведомления = Перечисления.ЭтапыУведомлений.Отказ И НЕ Настройка.ШаблонПисьма.Пустая() Тогда
...
Процедура ЗаписатьУведомлениеКандидатаВРегистр(Вакансия, Кандидат, Уведомление)
	...
	НоваяЗапись.ЭтапУведомления = Перечисления.ЭтапыУведомлений.Отказ;
