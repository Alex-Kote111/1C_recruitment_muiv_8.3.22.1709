...
&НаКлиенте
Процедура СформироватьОтчетСтатусКандидатов(Команда)
	
	СтрПараметры = Новый Структура;
	СтрПараметры.Вставить("Вакансия", Объект.Ссылка);
	
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("СтатусКандидатов", СтрПараметры);
		
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, СформироватьПриОткрытии", ПользовательскиеНастройки, Истина);
	
	ОткрытьФорму("Отчет.СтатусКандидатов.ФормаОбъекта", ПараметрыФормы); 
	
КонецПроцедуры

&НаКлиенте
Процедура СформироватьОтчетВоронкаКандидатов(Команда)
	
	СтрПараметры = Новый Структура;
	СтрПараметры.Вставить("Вакансия", Объект.Ссылка);
	
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("ВоронкаКандидатов", СтрПараметры);
		
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, СформироватьПриОткрытии", ПользовательскиеНастройки, Истина);
	
	ОткрытьФорму("Отчет.ВоронкаКандидатов.ФормаОбъекта", ПараметрыФормы); 	
	
КонецПроцедуры

&НаКлиенте
Процедура СформироватьОтчетСуммаЗатратНаВакансию(Команда)
	
	СтрПараметры = Новый Структура;
	СтрПараметры.Вставить("Вакансия", Объект.Ссылка);
	
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("СуммаЗатратНаВакансии", СтрПараметры);
		
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, СформироватьПриОткрытии", ПользовательскиеНастройки, Истина);
	
	ОткрытьФорму("Отчет.СуммаЗатратНаВакансии.ФормаОбъекта", ПараметрыФормы); 		
...
&НаКлиенте
Асинх Процедура ПосмотретьВакансиюНаСайте(Команда)
	
	//http://taukpebkr3.temp.swtest.ru/detailed_vacancy_page.php?vacancy_number=000000002
	
	ОчиститьСообщения();
	
	Если НЕ Объект.ВакансияРазмещенаНаСайте Тогда
		ОбщегоНазначенияКлиент.ВывестиСообщение("Вакансия не размещена на сайте");
		Возврат;
	КонецЕсли;
	
	Ждать ЗапуститьПриложениеАсинх("http://taukpebkr3.temp.swtest.ru/detailed_vacancy_page.php?vacancy_number=" + Объект.Номер);
