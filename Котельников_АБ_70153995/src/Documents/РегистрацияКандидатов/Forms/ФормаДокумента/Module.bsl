...
Процедура КандидатыПередУдалением(Элемент, Отказ)
	...
	Для Каждого ВыделеннаяСтрока Из Элемент.ВыделенныеСтроки Цикл		
		СтрокаТЧ = Объект.Кандидаты.Получить(ВыделеннаяСтрока);
		Если НЕ Элемент.ТекущиеДанные.Кандидат.Пустая() Тогда
			ИзменитьСтатусКандидата(СтрокаТЧ.Кандидат, ПредопределенноеЗначение("Перечисление.ЭтапыПодбора.Неразобранные"));
...
Процедура КандидатыКандидатПриИзменении(Элемент)
	...
	Если ТекущаяСтрокаТЧ.Кандидат.Пустая() Тогда
		Возврат;
	КонецЕсли;
	...
	ТекущаяСтрокаТЧ.ДатаПоступления = ДанныеКандидата.ДатаПоступления;
...
&НаКлиенте
Процедура КандидатыКандидатНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;
	
	Если НЕ Элементы.Кандидаты.ТекущиеДанные.Кандидат.Пустая() Тогда
		ОбщегоНазначенияКлиент.ВывестиСообщение("Вы уже выбрали кандидата. Если выбор был ошибочным, то нажмите на кнопку очистки и добавьте другого кандидата.", "Элемент");
		Возврат;
	КонецЕсли;
	
	ОтборКандидатов = Новый Структура("СсылкаНаВакансию, ЭтапПодбора", Объект.Вакансия, ПредопределенноеЗначение("Перечисление.ЭтапыПодбора.Неразобранные"));
...
&НаКлиенте
Процедура КандидатыКандидатОчистка(Элемент, СтандартнаяОбработка)
	...
	ИзменитьСтатусКандидата(ТекущаяСтрокаТЧ.Кандидат, ПредопределенноеЗначение("Перечисление.ЭтапыПодбора.Неразобранные"));
	ОповеститьОбИзменении(Тип("СправочникСсылка.Кандидаты"));
...
Функция ПолучитьДанныеКандидата(Кандидат)
	...
	ИзменитьСтатусКандидата(Кандидат, Перечисления.ЭтапыПодбора.ПросмотрРезюме);
	...
	Запрос.Текст = 
		...
		|	Кандидаты.ДатаПоступления КАК ДатаПоступления
		|ИЗ
	...
	ДанныеКандидата = Новый Структура("ИсточникПривлечения, ОткликОтКандидата, ДатаПоступления", Выборка.ИсточникПривлечения, Выборка.ОткликОтКандидата, Выборка.ДатаПоступления);
...
Асинх Процедура ОтправитьУведомлениеОРассмотренииРезюме(ТекущаяСтрокаТЧ)
	...
	Если ДанныеДляОтправкиПисьма.ШаблонПисьма.Пустая() Тогда
		ОбщегоНазначенияКлиент.ВывестиСообщение("Не найден шаблон письма о рассмотрении резюме. Проверьте свои настройки пользователя",, Объект.АвторДокумента);
		Возврат;
	КонецЕсли;
	...
	Если ПустаяСтрока(ДанныеДляОтправкиПисьма.ПочтаКандидата) Тогда
		ОбщегоНазначенияКлиент.ВывестиСообщение("У данного кандидата не заполнена почта. Отправка невозможна",, ТекущаяСтрокаТЧ.Кандидат);
		Возврат;
	КонецЕсли;
