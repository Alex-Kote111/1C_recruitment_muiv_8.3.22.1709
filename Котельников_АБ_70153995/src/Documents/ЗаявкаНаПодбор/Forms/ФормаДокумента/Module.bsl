...
Асинх Процедура ЗаполнитьИзПрофиляДолжности(Команда) 
	...
	Если Объект.ПрофильДолжности.Пустая() Тогда
		ОбщегоНазначенияКлиент.ВывестиСообщение("Выберите профиль должности", "Объект.ПрофильДолжности");
...
Функция ПолучитьДанныеПрофиляДолжности(ПрофильДолжности)
	...
	// Данные ТЧ "КлючевыеНавыки" 
	КлючевыеНавыки = ПрофильДолжности.КлючевыеНавыки;
	СписокНавыков = Новый Массив;
	
	Для Каждого СтрокаТЧ Из КлючевыеНавыки Цикл
	...
	КонецЦикла;
	ДанныеПрофиляДолжности.Вставить("КлючевыеНавыки", СписокНавыков);
...
Процедура ЗаполнитьЗаявкуДаннымиПрофиля(ДанныеПрофиляДолжности)
	...
	СписокНавыков = ДанныеПрофиляДолжности.КлючевыеНавыки;
	Для Каждого КлючевойНавык Из СписокНавыков Цикл
		СтрокаТЧ = Объект.КлючевыеНавыки.Добавить();
		СтрокаТЧ.Навык = КлючевойНавык.Навык;
		СтрокаТЧ.УровеньВладения = КлючевойНавык.УровеньВладения;
	КонецЦикла;
